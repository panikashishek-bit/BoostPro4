...
import type { Counter, CounterValue } from "./types";
 ...
 */
export async function collectCounters(env: DashboardEnv): Promise<CollectedCounter[]> {
  return Promise.all(
    COUNTERS.map(async (counter) => {
      try {
        return { counter, value: await counter.collect(env) };
      } catch (error) {
